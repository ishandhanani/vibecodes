...
fn render_title(f: &mut Frame, app: &App, area: ratatui::layout::Rect) {
    let (title_text, title_color) = match &app.tunnel_type {
        TunnelType::Ssh { .. } => ("⚡ SSH PORT FORWARD", Color::Rgb(0, 255, 255)),
        TunnelType::Kubernetes { .. } => ("☸ K8S PORT FORWARD", Color::Rgb(50, 150, 255)),
    };
...
fn render_host(f: &mut Frame, app: &App, area: ratatui::layout::Rect) {
    let active_tunnels = app.tunnel_processes.iter().filter(|p| p.is_some()).count();
    let host_status = if active_tunnels > 0 { "●" } else { "○" };
    let host_color = if active_tunnels > 0 {
        Color::Rgb(0, 255, 150)
    } else {
        Color::Gray
    };

    let type_badge_color = match &app.tunnel_type {
        TunnelType::Ssh { .. } => Color::Rgb(0, 200, 255),
        TunnelType::Kubernetes { .. } => Color::Rgb(50, 150, 255),
    };
...
fn render_ports(f: &mut Frame, app: &App, area: ratatui::layout::Rect) {
    let port_items: Vec<ListItem> = app
        ...
        .iter()
        .enumerate()
        .map(|(idx, port_mapping)| {
            let is_tunnel_active = app.tunnel_processes[idx].is_some();
            ...
            // Format remote target based on tunnel type
            let remote_target = match &app.tunnel_type {
                TunnelType::Ssh { host } => {
                    let host_short = host.split('@').last().unwrap_or(host);
                    format!("{}:{}", host_short, remote_port)
                }
                TunnelType::Kubernetes { resource, .. } => {
                    format!("{}:{}", resource, remote_port)
                }
            };
            ...
            let line_spans = vec![
                ...
                Span::styled(
                    remote_target,
                    Style::default().fg(if is_tunnel_active {
    ...
    let ports_list = List::new(port_items).block(
        Block::default()
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Rgb(80, 80, 120)))
            .title("TUNNELS"),
